...
public class EnderecoResource {
    ...
    @Operation(summary = "Listar Endereços", description = "Recuperar uma lista completa de endereços com todos os dados")
    ...
    @GET
    @Path("{id}")
    @Operation(summary = "Recuperar Endereço", description = "Recuperar apenas um endereço a partir de seu id")
    ...
    @Operation(summary = "Criar Endereços", description = "Cria um endereço completo")
    public Response criar(@Valid Endereco endereco) {
        ...
        return Response
                .status(Response.Status.CREATED).entity(endereco).build();
    }
    ...
    @DELETE
    @Path("{id}")
    @Operation(summary = "Deletar um Endereço", description = "Deletar um endereço a partir de seu id")
    ...
    @Operation(summary = "Atualizar status de Endereço", description = "Ativa ou desativa um endereço", requestBody = @RequestBody(description = "Status de endereço", required = true, content = @Content(schema = @Schema(implementation = EnderecoStatus.class))))
    public Response mudarStatus(@PathParam("id") Long id, String status) {
        try{
            endereco = enderecoService.mudarStatus(id, EnderecoStatus.fromString(status));
        }catch(Exception e){
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(e.getMessage()).type("text/plain").build();
        }
